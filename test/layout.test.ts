...
test("short user line and wrapped user turn share one left column; assistant starts at 0", () => {
  ...
  assert.equal(
    userBodies.some((row) => /^\s+\S/.test(row.text) && row.text.trimStart().length < longest),
    false,
    "must not padStart each wrapped line to the pane",
  );
...
test("user text that fits the bubble inner width is a single left-aligned line", () => {
...
test("only user strings longer than the bubble inner width wrap", () => {
